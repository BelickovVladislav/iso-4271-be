...
class PrestartPostgreSQL {
  ...
  constructor() {
    ...
    this._dataSource = new DataSource({ ...AppDataSource.options, database: 'template1' } as PostgresConnectionOptions);
  }
  ...
  private async _createDBIfNotExists(): Promise<void> {
    try {
      const result = await this._dataSource.query('SELECT datname FROM pg_database');
      const isExists = result.some((row: QueryResultRow) => row['datname'] === this._database);
  ...
  private async _connection(): Promise<void> {
    try {
      ...
      PrestartPostgreSQL.logger.error(`CONNECTION ERROR: ${e}`.replace(' error: ', ' '));
      await this._disconnect(1);
    }
  }
}
